...
import parser from "neo4j-parser";
import QueryObj from "neo4j-query-object";
import tools from "cypher-tools";
 ...
 * new simple-neo4j()
 */
module.exports = class SimpleNeo4J {
    ...
    update(opts, reset = false)
    {
        ...
        if(_opts.transactionQueue || reset)
            this.transactionQueue = _opts.transactionQueue;

        if(_opts.queryQueue || reset)
            this.queryQueue = _opts.queryQueue;

        if(_opts.url || reset)
            this.url = _opts.url;

        if(_opts.username || reset)
            this.username = _opts.username;

        if(_opts.password || reset)
            this.password = _opts.password;

        if(_opts.transaction || reset)
            this.transaction = _opts.transaction;

        if(_opts.QueryObjectClass || reset)
            this.QueryObjectClass = _opts.QueryObjectClass;
    }
    ...
    set queryQueue(value)
    {
        if(typeof value === `object`
            && value !== null)
            this._queryQueue = value;
        else
            this._queryQueue = {};
    }
    ...
    set url(value)
    {
        ...
        const urlTest = /^http[s]?\:\/\/.*?\:[1-9][0-9]*$/;

        if(typeof value === `string`
            && value.length > 0
            && urlTest.test(value))
            this._url = value;
        else this._url = `http://localhost:7474`;
    }
     ...
     */
      ...
      */
       ...
       * @since 4.1.3
       */
       get QueryObjectClass()
       {
           return this._QueryObjectClass;
       }

       set QueryObjectClass(value)
       {
           if(value instanceof QueryObj)
               this._QueryObjectClass = value;
           else
               this._QueryObjectClass = QueryObj;
       }
    ...
    nextInQueue()
    {
        ...
        || this.transactionQueue.length < 1) return;
        ...
        while(true)
        {
            ...
            if(Array.isArray(transaction)
                && transaction.length > 1) break;
            else if(this.transactionQueue.length < 1) return;
        }
        ...
        for(let i = 0; i < transaction.length; i++)
        {
            ...
            if(!(transaction[i] instanceof QueryObj)) continue;
     ...
     * // "Hello"
     */
    addQuery(opts, queueName = `default`) {
        ...
        const queryObjs = this.processQueries(opts);
        ...
        if(typeof _queueName !== `string`
            || _queueName.length < 1) _queueName = `default`;

        if(!Array.isArray(this.queryQueue[_queueName]))
    ...
    processQueries(opts)
    {
        ...
        if(!Array.isArray(opts)) _opts.push(opts);
        else _opts = _opts.concat(opts);
    ...
    reportErrors(error) {
        ...
        for(let i = 0; i < this.transaction.length; i++) {
            const queryObj = this.transaction[i];
     ...
     * this.commit();
     */
    commit(raw = false, queryQueueName = `default`)
    {
        let _queueName = queryQueueName;
        if(typeof _queueName !== `string`
            || _queueName.length < 1) _queueName = `default`;
        ...
        for(let i = 0; i < this.queryQueue[_queueName].length; i++)
        {
            const queryItem = this.queryQueue[_queueName][i];

            // Only valid and filled queries will be added to the transaction
            if(!(queryItem instanceof QueryObj)
                || queryItem.query.length < 1) continue;
    ...
    send(raw = false) {
        ...
        for(let i = 0; i < this.transaction.length; i++)
        {
            const query = this.transaction[i];
        ...
        (error, response, body) => {
            ...
            parser({
                ...
                cb: (err, index, resultObj) =>
                {
                    ...
                    const queryObj = this.transaction[index];
                    if(!(queryObj instanceof QueryObj)) return;
